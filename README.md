...
<p align="center">
  为母语为汉语的外国人设计的汉字记忆与拼音输入肌肉记忆锻炼软件
</p>
...
软件设计的目标群体为母语为汉语的外国人。学习汉语时，记忆汉字和拼音常常是一大挑战。为了巩固汉字技能，需要持续的背诵和练习 📕。

本软件将汉字的记忆与拼音键盘输入的肌肉记忆的锻炼相结合，可以在背诵汉字的同时巩固肌肉记忆。通过拼音输入汉字，加深对发音和字形的理解。

为了避免造成错误的肌肉记忆，设计上如果用户拼音输入错误则需要重新输入，尽可能确保用户维持正确的拼音输入习惯，从而更快掌握汉语的发音与拼写。

软件也对正在准备参加汉语水平考试（如 HSK）的人群有一定的帮助。
...
内置了常用的 HSK（汉语水平考试） 各级别词库，以及生活常见汉语词汇库。尽可能满足大部分汉字学习者对记忆汉语词汇的需求，也非常欢迎社区贡献更多的词库。
<br />
<br />
...
以算法著称，功能非常完善的打字网站，根据用户输入每个字母的正确率与速度生成“伪汉语”来帮助用户集中锻炼个别输入较慢的字母。并可以根据用户的输入记录生成完整的分析报告。

也是本项目的核心灵感来源，Keybr 更多针对汉语为母语的用户。在我使用 Keybr 练习打字时，觉得虽然生成的伪汉语能够练习输入不顺畅的个别字母，但并不能提升非母语用户对单词的掌握，于是有了本项目。
